#!/usr/bin/env python3
"""Stage one or more Codex CN npm packages for release."""
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in CODEX_PLATFORM_PACKAGES:
        ...
        return f"codex-cn-npm-{platform}-{version}.tgz"
    if package == "codex":
        return f"codex-cn-npm-{version}.tgz"
    if package == "codex-responses-api-proxy":
        return f"codex-cn-responses-api-proxy-npm-{version}.tgz"
    if package == "codex-sdk":
        return f"codex-cn-sdk-npm-{version}.tgz"
