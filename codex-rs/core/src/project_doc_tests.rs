...
async fn js_repl_instructions_are_appended_when_enabled() {
    ...
    assert!(res.contains("`js_repl` is a freeform/custom tool."));
    assert!(res.contains("Use `console.log`, `codex.tool(...)`, and `codex.emitImage(...)`."));
}

#[tokio::test]
async fn chat_provider_js_repl_instructions_describe_function_wrapper() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let mut cfg = make_config(&tmp, 4096, None).await;
    cfg.features
        .enable(Feature::JsRepl)
        .expect("test config should allow js_repl");
    cfg.model_provider_id = "deepseek".to_string();
    let mut providers = built_in_model_providers(/* openai_base_url */ None);
    cfg.model_provider = providers
        .remove("deepseek")
        .expect("deepseek provider should exist");

    let res = get_user_instructions(&cfg)
        .await
        .expect("chat provider js_repl instructions expected");
    assert!(
        res.contains("On chat-completions providers, `js_repl` is exposed as a function tool.")
    );
    assert!(res.contains("Put the full JavaScript program in the `code` field"));
}
...
async fn js_repl_tools_only_instructions_are_feature_gated() {
    ...
    let res = get_user_instructions(&cfg)
        ...
        .expect("js_repl instructions expected");
    assert!(res.contains(
        "Do not call tools directly; use `js_repl` + `codex.tool(...)` for all tool calls"
    ));
    assert!(res.contains("MCP tools (if any) can also be called by name via `codex.tool(...)`."));
}
...
async fn js_repl_image_detail_original_does_not_change_instructions() {
    ...
    let res = get_user_instructions(&cfg)
        .await
        .expect("js_repl instructions expected");
    assert!(res.contains("Request full-resolution image processing with `detail: \"original\"`"));
    assert!(res.contains("Top-level static import declarations"));
}
