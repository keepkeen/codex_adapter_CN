...
async fn assert_manual_compact_smoke(case: LiveProviderCase, test_name: &str) -> Result<()> {
    ...
    let result = run_live_manual_compact(case, LiveTuiImplementation::AppServer, &api_key).await?;
    assert!(
        result.seed_stdout.contains("provider-live-compact-line-64"),
        "expected seeded exec thread output for {} app-server /compact smoke\nseed stdout:\n{}\nseed stderr:\n{}",
        case.provider_id,
        result.seed_stdout,
        result.seed_stderr
    );
    assert!(
        ...
        "expected /compact op in session log for {} app-server /compact smoke\npty output:\n{}",
        case.provider_id,
        result.output
    );
    assert!(
        result.contains_context_compacted_event()?,
        "expected context_compacted event in session log for {} app-server /compact smoke\npty output:\n{}",
        case.provider_id,
        result.output
    );
    assert!(
        result.contains_rollout_compacted_item()?,
        "expected compacted rollout item for {} app-server /compact smoke\npty output:\n{}",
        case.provider_id,
        result.output
    );
...
provider_live_compact_tests!(
    DEEPSEEK_PROVIDER,
    deepseek_app_server_live_manual_compact_smoke
);
provider_live_compact_tests!(GLM_PROVIDER, glm_app_server_live_manual_compact_smoke);
provider_live_compact_tests!(KIMI_PROVIDER, kimi_app_server_live_manual_compact_smoke);
provider_live_compact_tests!(
    MINIMAX_PROVIDER,
    minimax_app_server_live_manual_compact_smoke
);
